...
app.get('/debug', async (req, res) => {
  try {
    ...
    // Lista grupos/workspaces acessíveis
    ...
    res.json({
      ...
      token_scp: payload.scp || '',
      token_appid: payload.appid,
      token_oid: payload.oid,
      workspaces_status: wsResp.status,
      workspaces: wsData
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
